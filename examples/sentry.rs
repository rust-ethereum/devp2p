...
async fn main() {
    ...
    let discv5_secret_key = enr::CombinedKey::Secp256k1(
        libsecp256k1_03::SecretKey::parse(&secret_key.serialize()).unwrap(),
    );

    let local_enr = enr::EnrBuilder::new("v4")
        .build(&discv5_secret_key)
        .unwrap();
    let mut discovery =
        discv5::Discv5::new(local_enr, discv5_secret_key, Default::default()).unwrap();

    discovery.start("0.0.0.0:30303".parse().unwrap()).unwrap();

    discovery.add_enr("enr:-Je4QLwChs4yCRsR7BzunVuG0oiX9pgm9sn3qQKBBAtA9m3ddikRDjs5choQFdeiGZxebfisi6LEQsdM-GGXAX0W-bYDg2V0aMfGhOAp6ZGAgmlkgnY0gmlwhBKx4DCJc2VjcDI1NmsxoQOTdVbFqEYqnyzNnSux_LOBW92hlRwRo4lYA3uPIbhRl4N0Y3CCdl-DdWRwgnZf".parse().unwrap());

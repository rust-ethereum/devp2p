...
impl ECIES {
    ...
    fn encrypt_message(&self, data: &[u8]) -> Vec<u8> {
        ...
        encryptor.encrypt(&mut RefReadBuffer::new(data),
                          &mut RefWriteBuffer::new(&mut iv_encrypted[16..]), true).unwrap();
    ...
    fn decrypt_message(&self, encrypted: &[u8]) -> Vec<u8> {
        let public_key = PublicKey::from_slice(&SECP256K1, &encrypted[0..65]).unwrap();
        ...
        encryptor.decrypt(&mut RefReadBuffer::new(&encrypted_data),
                          &mut RefWriteBuffer::new(&mut decrypted_data), true).unwrap();
    ...
    fn parse_auth_unencrypted(&mut self, data: [u8; AUTH_LEN]) {
        let sig_rec = RecoverableSignature::from_compact(
            &SECP256K1, &data[0..64], RecoveryId::from_i32(data[64] as i32).unwrap()).unwrap();
        ...
        let msg = Message::from_slice((x ^ self.remote_nonce.unwrap()).as_ref()).unwrap();
    ...
    pub fn parse_header(&mut self, data: &[u8]) -> usize {
        ...
        self.body_size = Some(decrypted.as_ref().read_uint::<BigEndian>(3).unwrap() as usize);
...
mod tests {
    ...
    fn communicate() {
        ...
        let mut server_ecies = ECIES::new_server(server_secret_key);
        let mut client_ecies = ECIES::new_client(client_secret_key, pk2id(&server_public_key));

        // Handshake
        server_ecies.parse_auth(client_ecies.create_auth().as_ref());
        client_ecies.parse_ack(server_ecies.create_ack().as_ref());

        let server_to_client_data = [0u8, 1u8, 2u8, 3u8, 4u8];
        let client_to_server_data = [5u8, 6u8, 7u8];

        // Test server to client 1
        client_ecies.parse_header(server_ecies.create_header(server_to_client_data.len()).as_ref());
        let ret = client_ecies.parse_body(server_ecies.create_body(&server_to_client_data).as_ref());
        assert_eq!(ret, server_to_client_data);

        // Test client to server 1
        server_ecies.parse_header(client_ecies.create_header(client_to_server_data.len()).as_ref());
        let ret = server_ecies.parse_body(client_ecies.create_body(&client_to_server_data).as_ref());
